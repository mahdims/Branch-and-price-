...
def Calculate_the_subproblem_obj(Data, R, Duals, col, q):
    ...
    for e in Pi6.keys():
        ...
        if e[0] in col.route and e[1] not in col.route:
            uuu = 0
...
def Columns_with_negitive_costs(Data, R, Duals, Col_dic):
    ...
    for col_ID, col in Col_dic.items():
        ...
        New_RDP = [0] + list(NewQ) # in GRASP  we only have the q for customers but here in ColGen node zero should have 0 deleivery
...
def ColumnGen(Data, R, RMP, G_in, Col_dic, dis, edges2keep, edges2avoid, (Sub,x,q,a)):
    ...
    Subobj = -1
    counter = 0 # we move onb to the optimal solver when 3 sccesive huistic can't improve the problem.
    Hit_the_Zero = 0 # become one for the first time that the huristic find a solution with value zero after that we use
    # the optimal solver only
    Pre_Sub_obj = -10
    Heuristic_works = 1
    ...
    while Subobj < Stoping_R_cost: # Stoping critira !
        ...
        # For each already existing routes in the col set we calculate the delivery quantity by the heuristic approach.
        ...
        if not we_found_cols or not Heuristic_works:
            ...
            if Heuristic_works: # Solve it with the heuristics
                ...
                if Heuristic_works:
                    #print("Sub Problem heuristics value: %f" %heuristic_path_value)
                    Solved_by_Heuristic = 1# the original in 1
        ...
        if counter >= 3:
            counter = 0
            #Heuristic_works = 0 # should be enable
        # update the previous heuristic value
        if heuristic_path_value:
            Pre_Sub_obj = heuristic_path_value

        if heuristic_path_value > Stoping_R_cost:
            heuristic_path_value = -10
            Heuristic_works = 0 # change to 0 after the expriment
