...
def MasterModel(Data, Col_dic, R):
    ...
    RMP.update()
    RMP.setObjective(quicksum(Gc.nodes[i]['demand'] - quicksum(Col_dic[r].RDP[q][i]*y[r, q]
                                                               for r, q in y.keys()) for i in Gc.nodes)
                    + (Lambda/TotalD) * quicksum(tp[i, j]+tn[i, j] for i, j in tp.keys())
                    + (Gamma / R) * (Data.Total_dis_epsilon - quicksum(Col_dic[r].travel_time * y[r, q]
                                                                       for r, q in y.keys())))
    ...
    TotalTime = RMP.addConstr(quicksum(y[r, q]*Col_dic[r].travel_time
                                       for r, q in y.keys()) <= Data.Total_dis_epsilon, name="Total_time")
    visit = RMP.addConstrs((quicksum((Col_dic[r].is_visit(i) != -1) * y[r, q]
                                     for r, q in y.keys()) == 1 for i in Gc.nodes), name="visit")
    vehicle = RMP.addConstr(quicksum(y[r, q]
                                     for r, q in y.keys()) <= M, name="vehicle")
    Inv = RMP.addConstr(quicksum(y[r, q]*sum(Col_dic[r].RDP[q])
                                 for r, q in y.keys()) <= G.nodes[0]['supply'] +1, name="Inv")
