...
class Node():
    ...
    def __init__(self, ID,PID ,level, pos ,Col_dic,G,edge2keep= [] ,edge2avoid= [],Initial_RDP=None):
        ...
        if self.feasible :
            #self.Update_shortest_dis() # graph G is changed so the shortest paths 
    ...
    def Col_dic_update(self,RDP):
        ...
        if self.Col_dic:
            MaxNo=max(self.Col_dic.keys())
        else:
            MaxNo=1
        for inx,R_D in enumerate(New_Col):
            self.Col_dic[MaxNo+inx+1] = copy.copy(R_D)         
    ...
    def solve(self):
        ...
        start=time()
        (self.feasible, self.Objval, self.Y, self.Col_dic) = ColumnGen(Node.Data,Node.R,RMP,self.G,self.Col_dic,self.Dis,self.edges2keep,edges2avoid,sub)
        ...
        if self.feasible: # find the used route-deliveries
            ...
            self.selected_route = [self.Col_dic[i].route  for i in self.selected_R_D]
            #self.Rounding()
    ...
    def Strong_Branching(self):
        # Find the barnchnig rule
        Edge_Val={}
        # calculate the xij values ***** Not for   i--NN+1 ******
        
        for edge in self.G.edges():
            if Node.Data.NN+1 not in edge: #Do not branch on i-NN+1 edges
                Edge_Val[edge]= abs( sum([ edge_in_route(edge , self.Col_dic[inx].route)* self.Y[inx] for inx in self.selected_R_D ]) - 0.5)
    ...
    def Arc_Branching(self,WhichNode,i,j):
        ...
        if WhichNode=="left":
            ...
            # connectivity check delete after you done !
