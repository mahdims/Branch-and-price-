...
def Model1_V2(Data,R):
    ...
    v = VF_MIP.addVars(G.node ,lb=0,name="v")
    w = VF_MIP.addVars(G.node ,lb=0,name="w")
    u = VF_MIP.addVars(G.node ,lb=0,name="u") 
    ...
    VF_MIP.addConstrs(quicksum( x.select(i , '*') )  == quicksum(x.select('*',i) ) for i in Gc.node )
    VF_MIP.addConstrs(quicksum(x.select( 0 , '*') )== M for k in range(M) )
    VF_MIP.addConstrs(quicksum(x.select('*',NN+1) )== M  for k in range(M) )
    VF_MIP.addConstrs(v[i]<=G.node[i]['demand'] for i in Gc.node  )
    VF_MIP.addConstr(quicksum(v)<=G.node[0]['supply'])
    VF_MIP.addConstrs(u[i] <= Data.Maxtour for i in Gc.node )
    VF_MIP.addConstrs(w[i] <= Q for i in Gc.node )
    #VF_MIP.addConstr( w[NN+1] <= Q)
    ...
    if VF_MIP.status==2:
        ...
        print("Maximum tour lenght : %s" %Data.Maxtour)
        print("Vehilce capacity : %s" %Q)
        for r in routes:
            RDP=[0]
            for i in r[1:-1]:
