...
from Pricing_GRASP import GRASP
import copy
import sys
...
def ColumnGen(Data,R,RMP,G_in,Col_dic,dis,edges2keep,edges2avoid,(Sub,x,q,a)):
    ...
    while Subobj< Stoping_R_cost: # Stoping critira !
        ...
        ### Solve Sub Problem
        # First try to solve it with the huristics and then if not a chance to find a solution
        ( pricing_indi , heuristic_path , heuristic_q ) = GRASP(Data, edges2keep, edges2avoid, Duals)
        if pricing_indi ==0 :
            ...
            Xv=Sub.getAttr('x',x)
            qv=Sub.getAttr('x',q)
            print([G.node[i]['demand'] for i in G.nodes])
            print(qv)
            ...
            New_Route = Route(Data,heuristic_path)
            New_Route.set_RDP(heuristic_q )
            pre_node=0
        ...
        RMP.addVar(lb=0,ub=1,obj=-sum(New_Route.RDP)-Gamma*New_Route.travel_time/R,name="y[%d]" %(newY_inx), column=col )
        RMP.update()
        # Update path
