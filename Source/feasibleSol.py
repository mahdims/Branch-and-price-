...
def TabuRoute(Data,length,edges2keep,Pars,paths):
    it = 1
...
def Initial_feasibleSol(Data2,G,edges2keep=None,edges2avoid=None,RDP=None):
    if RDP is not None:
        Route.Class_RDP=0.8*RDP
    Route.E2keep(edges2keep)
    Route.edges2avoid=edges2avoid
    global Data
    Data=Data2
    ...
    while initial_Sol_count <= 1 :
        ...
        for p in paths:
            p.set_RDP()
        if edges2keep:
            paths = relocate_edges(paths,edges2keep)

        if any([0 in r.route[1:-1] for r in paths]):
            SSS=0
        ...
        if inf_count>=4:
            ...
            #nx.draw_networkx_edges( G ,pos, edgelist = G.edges(), width=2, edge_color='#000000')
            #print(edges2keep)
            #plt.show()
            return ([] , 0) 
