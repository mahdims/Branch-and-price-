...
def SubProblem(Data,G,dis,edges2keep=None, edges2avoid=None):
    ...
    #[(j,i) for i,j in G.edges() if i!=0 and j!=NN+1] + [(i,j) for i,j in G.edges()]    
    ...
    if edges2keep:
        for edge in edges2keep:
            if edge[0]==0:
                Sub.addConstr( x[edge[0],edge[1]] == a[ edge[1] ] ) 
            elif edge[1]==NN+1:
                sys.exit("Branch on i-NN+1 -- Subproblem")
                Sub.addConstr( x[edge[0],edge[1]]  == a[ edge[0] ] ) 
            else:
