...
def print_updates(start, Filename, ncol):
    ...
    print(data_format.format(round(Node.LowerBound, 2), round(Node.UpperBound, 2), Node.Gap, Elapsed_time, Node.time2UB, Node.NodeCount, round(avg(ncol),2)))
...
def branch_and_bound(Data, MaxTime, Filename):
    ...
    if root.integer(): # check if we need to continue
        ...
        Node.reset()
        return str(round(UB,3)), str(round(LB,3)), str(round(Gap,3)), str(round(time2UB,3)), str(round(Elapsed_time,3)), str(NCount), str(round(avg(ncol),3))
