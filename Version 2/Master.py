...
def MasterModel(Data,Col_dic,R,edge2keep=None):
    ...
    RMP.setObjective( quicksum( Gc.node[i]['demand'] - quicksum(Col_dic[r].RDP[q][i]*y[r,q] for r,q in y.keys() ) for i in Gc.nodes)
    ...
    linear = RMP.addConstrs((tp[i,j]-tn[i,j]+
        quicksum(Col_dic[r].RDP[q][i]*y[r,q]*G.node[j]['demand'] for r,q in y.keys()  )
        - quicksum(Col_dic[r].RDP[q][j]*y[r,q]*G.node[i]['demand'] for r,q in y.keys() )
    ...
    Inv = RMP.addConstr(quicksum(y[r,q]*sum(Col_dic[r].RDP[q]) for r,q in y.keys() ) <= G.node[0]['supply'], name="Inv" )
